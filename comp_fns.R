number_to_comp <- function(number) {
  ...
  content$body %>% html_nodes("#subheading > h1") %>% 
    html_text() -> comp_name
  content$body %>% html_nodes("#submenu > ul > li.current > a") %>% 
    html_attr("href") %>% str_split("/") %>% pluck(1) -> v
  list(error = FALSE, class = v[2], where = v[3], comp_name = comp_name, season = v[5], comp_part = v[6])
}
